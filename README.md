...
- **백그라운드에서 조용히 색인** — CPU·디스크 I/O 우선순위를 모두 낮추고, CPU 점유도 짧게 몰아 쓰지 않고 잘게 나눠 쓰도록 최적화되어, 대량 색인 중에도 다른 작업(탐색기, 검색 요청 등)이 방해받지 않습니다.
...
대량 문서를 색인하는 동안에도 다른 작업이 원활하도록 안정성을 집중적으로 개선했습니다.

- **v1.2.2** — 색인 작업의 CPU 점유 방식 개선. 실제 연산량은 그대로 두고, 짧고 굵게 몰아 쓰던 CPU를 잘게 나눠 쓰도록 스로틀링을 적용해 대량 색인 중에도 트레이 앱 상태 조회 등이 끊기지 않습니다.
- **v1.2.1** — 색인 프로세스와 검색 프로세스가 공유하는 데이터베이스에서 발생하던 락 경합을 완화해, 대량 색인 중 검색 응답이 지연되던 문제를 수정했습니다(40~111초 → 8.5초).
- **v1.2.0** — 대량 재색인 중 디스크 I/O 우선순위 문제로 시스템 전체(Windows 탐색기 포함)가 응답 없어지던 문제를 수정했습니다.
